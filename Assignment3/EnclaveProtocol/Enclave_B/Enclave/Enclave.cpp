...
/*************************
 * BEGIN [1. Bob receives (and checks) PSK from Alice]
 *************************/
...
/*************************
 * END [1. Bob receives (and checks) PSK from Alice]
 *************************/
...
/*************************
 * BEGIN [5. E_B decrypts challenge]
 *************************/

sgx_status_t getChallenge(uint8_t *encrypted_challenge, uint8_t *decrypted_challenge){

    sgx_status_t sgx_status;

    sgx_status = decrypt(encrypted_challenge, decrypted_challenge, 2);

    return sgx_status;
}

/*************************
 * END [5. E_B decrypts challenge]
 *************************/



/*************************
 * BEGIN [6. E_B computes (and encrypts) response]
 *************************/

sgx_status_t computeResponse(uint8_t *challenge, uint8_t *encrypted_response){

    sgx_status_t sgx_status;

    uint8_t a = challenge[0];
    uint8_t b = challenge[1];

    uint16_t c = a + b;

    // https://stackoverflow.com/questions/51555676/how-to-divide-an-int-into-two-bytes-in-c
    uint8_t lsb = (uint8_t) (c & 0xff);
    uint8_t msb = (uint8_t) (c >> 8);

    uint8_t *response = (uint8_t *) malloc(2 * sizeof(uint8_t));
    response[0] = msb;
    response[1] = lsb;

    sgx_status = encrypt(response, encrypted_response, 2);

    return sgx_status;
}

/*************************
 * END [6. E_B computes (and encrypts) response]
 *************************/
